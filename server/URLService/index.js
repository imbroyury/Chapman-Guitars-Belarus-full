...
export const getAllUrls = async () => {
  ...
  const itemUrisOfUrls = await series(
    dynamicUrlDeclarations.map(urlDeclaration => urlDeclaration.itemUrisFetcher)
  );
  ...
  const allUrls = [...staticUrls, ...calculatedUrls].map(url => HOST + url);
  return allUrls;
};